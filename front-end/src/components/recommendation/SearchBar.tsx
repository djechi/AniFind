...
const SearchBar = () => {
    ...
    const fetchRecommendations = async () => {
        ...
        const response = await fetch(`${API_BASE_URL}/recommendations?anime_id=${selectedAnime.mal_id}`);
        const data = await response.json();
        console.log("API Response:", data);

        if (data.data) {
            const recommendedAnime = data.data.map(item => item.entry).slice(0,15);
    ...
    return(
        <div className="flex flex-col mb-[30vh] items-center justify-center w-full h-full">
                <SearchTitle />
                ...
                <div className="flex flex-row mx-auto bg-ani-grey-300 rounded-2xl w-[20vw] h-[6vh] py-2 px-3 mt-[3vh] relative">
                    ...
                    >
                        <img src = { SearchButtonImage } alt="Search Button Image" className = "w-[2vw] h-[3vh]"/>
                    </button>
                ...
                <div className= "mt-[20vh] grid grid-cols-3 gap-y-[20vh] gap-x-[15vw]">
